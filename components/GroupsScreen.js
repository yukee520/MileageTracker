...
const GroupsScreen = ({ user, onRefresh, refreshKey, pendingRequestsCount, onPendingUpdate }) => {
  ...
  const [showMemberTrips, setShowMemberTrips] = useState(false);
  ...
  const loadGroupData = async () => {
    ...
    try {
      ...
      const { data: profile, error: profileError } = await supabase
        .from('profiles')
        .select('team_id, role, subscription_tier, subscription_expiry')
        .eq('id', user.id)
        .single();
      ...
      const isLeader = hasValidTeam && profile?.role === 'leader';

      if (isMounted.current) {
        setHasGroup(hasValidTeam);
        setIsGroupLeader(isLeader);
        console.log('👑 Is Group Leader:', isLeader);
      }

      if (hasValidTeam && profile?.team_id && isMounted.current) {
        const teamResult = await GroupService.getGroupDetails(profile.team_id);
        if (teamResult.success && isMounted.current) {
  ...
  useEffect(() => {
    ...
    if (!isMounted.current) return;
    console.log('🔄 GroupsScreen refreshing due to refreshKey:', refreshKey);
    loadGroupData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [refreshKey]);
  ...
  // ============================================================
  // PAID-PLAN CHECK (used to block dismissal)
  // ============================================================
  const hasActivePaidGroupPlan = () => {
    if (!groupInfo) return false;
    const tier = groupInfo.subscription_tier;
    const paidTiers = ['team_basic', 'team_pro'];
    if (!paidTiers.includes(tier)) return false;
    if (groupInfo.payment_status !== 'active') return false;
    const expiry = groupInfo.subscription_end_date ? new Date(groupInfo.subscription_end_date) : null;
  ...
  const handleViewMemberTrips = async (member) => {
    try {
      setLoading(true);
      ...
      setMemberTrips(data || []);
      setShowMemberTrips(true);
    } catch (error) {
      console.error('Error loading member trips:', error);
      Alert.alert('Error', 'Failed to load member trips');
    } finally {
      setLoading(false);
    }
  };
...
const { data: allTrips, error } = await query;
      ...
      if (!allTrips || allTrips.length === 0) {
        Alert.alert('No Data', 'No trips found for team members');
        setExporting(false);
        return;
      }

      const workbook = new ExcelJS.Workbook();
      workbook.creator = groupInfo?.name || 'Team Report';
      workbook.created = new Date();

      const sheet = workbook.addWorksheet('Team Trips');
      ...
      allTrips.forEach((trip, index) => {
        const row = sheet.addRow({
          ...
          member: trip.profiles?.full_name || trip.profiles?.email || 'Unknown',
      ...
      const fileName = `team_report_${groupInfo?.name || 'group'}_${timestamp}.xlsx`;

      let baseDir = FileSystem.documentDirectory || FileSystem.cacheDirectory || '/data/data/com.yourcompany.mileagetracker/cache/';
      ...
      if (canShare) {
        await Sharing.shareAsync(filePath, {
          ...
          dialogTitle: 'Export Team Report',
      ...
      Alert.alert('Success', 'Team report exported successfully!');
    } catch (error) {
      console.error('Error exporting team report:', error);
      Alert.alert('Error', 'Failed to export team report: ' + error.message);
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleDismissGroup = () => {
    if (hasActivePaidGroupPlan()) {
      const expiry = groupInfo.subscription_end_date
        ? new Date(groupInfo.subscription_end_date).toLocaleDateString()
        : 'unknown date';
      Alert.alert(
        'Cannot Dismiss Group',
        `This group has an active ${groupInfo.subscription_tier === 'team_pro' ? 'Group Pro' : 'Group Basic'} subscription until ${expiry}.\n\nYou can dismiss the group after the subscription expires.`
      );
      return;
    }
  ...
  const openTransferModal = () => {
    ...
    if (others.length === 0) {
      Alert.alert('No Members', 'You need at least one other member to transfer leadership.');
      return;
    }
    setShowTransferModal(true);
  };

  const handleTransferLeadership = (targetMember) => {
    Alert.alert(
      'Transfer Leadership',
      `Transfer leadership to ${targetMember.full_name || targetMember.email}?\n\nYou will become a regular member and lose leader privileges.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Transfer',
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Success', `${targetMember.full_name || targetMember.email} is now the group leader.`);
              await loadGroupData();
              if (onRefresh) onRefresh();
            } catch (error) {
  ...
  const handlePaymentSuccess = (tier, transactionDetails) => {
    console.log('✅ Payment successful for:', tier);
    setShowPaymentModal(false);
    Alert.alert('Success', `Your ${tier} subscription has been activated!`);
    loadGroupData();
    if (onRefresh) onRefresh();
  };
  ...
  const getLeader = () => {
    ...
    return leader?.full_name || leader?.email || 'Unknown';
  };

  const getMemberCount = () => groupMembers.length || 1;

  const getSubscriptionStatus = () => {
    if (!groupInfo) return 'Group Free';
    const tier = groupInfo.subscription_tier;
    if (tier === 'team_basic') return 'Group Basic';
    if (tier === 'team_pro') return 'Group Pro';
    return 'Group Free';
  };
        ...
        <View style={styles.noGroupContainer}>
          ...
          >
            ...
            <View style={styles.actionContent}>
              ...
              <Text style={styles.actionSubtitle}>Become a group leader</Text>
            </View>
          ...
          >
            ...
            <View style={styles.actionContent}>
              ...
              <Text style={styles.actionSubtitle}>Join an existing group</Text>
            </View>
            <Text style={styles.actionArrow}>→</Text>
          </TouchableOpacity>
        </View>
  ...
  return (
    ...
    >
      <View style={styles.groupHeader}>
          ...
          <View style={styles.headerActions}>
            ...
            {isGroupLeader ? (
              <TouchableOpacity
                ...
                disabled={processing}
              >
                <Text style={styles.iconDangerText}>🗑️</Text>
              </TouchableOpacity>
            ) : (
              <TouchableOpacity
                style={styles.iconDangerBtn}
        ...
        <Text style={styles.groupLeader}>👑 Leader: {getLeader()}</Text>
      ...
      <View style={styles.tabBar}>
        {isGroupLeader && (
          <TouchableOpacity
            style={[styles.tabBtn, activeSection === 'requests' && styles.tabBtnActive]}
            onPress={() => setActiveSection('requests')}
          >
            <Text style={[styles.tabBtnText, activeSection === 'requests' && styles.tabBtnTextActive]}>
              📨 Requests{pendingRequests.length > 0 ? ` (${pendingRequests.length})` : ''}
            </Text>
          </TouchableOpacity>
        )}
        <TouchableOpacity
          style={[styles.tabBtn, activeSection === 'members' && styles.tabBtnActive]}
          onPress={() => setActiveSection('members')}
        >
          <Text style={[styles.tabBtnText, activeSection === 'members' && styles.tabBtnTextActive]}>
            👥 Members ({groupMembers.length})
          </Text>
        </TouchableOpacity>
        {isGroupLeader && (
          <TouchableOpacity
            style={[styles.tabBtn, activeSection === 'subscription' && styles.tabBtnActive]}
            onPress={() => setActiveSection('subscription')}
          >
            <Text style={[styles.tabBtnText, activeSection === 'subscription' && styles.tabBtnTextActive]}>
              💳 Plan
            </Text>
          </TouchableOpacity>
        )}
      ...
      {isGroupLeader && activeSection === 'requests' && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>📨 Pending Requests ({pendingRequests.length})</Text>
          {pendingRequests.length === 0 ? (
            <Text style={{ color: '#888', paddingVertical: 8 }}>No pending requests.</Text>
          ) : (
          ...
          {groupMembers.map((member) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => {
                  if (isGroupLeader && member.id !== user.id) {
                    handleViewMemberTrips(member);
                  } else if (member.id === user.id) {
                    Alert.alert('Your Trips', 'View your trips in the History tab.');
                  } else {
                    Alert.alert('Member', 'Only group leaders can view member trips.');
                  }
                }}
                disabled={!isGroupLeader || member.id === user.id}
              >
                <View style={styles.memberInfo}>
                  ...
                  <Text style={[styles.memberRole, isMemberLeader && styles.leaderRoleText]}>
                ...
                <View style={styles.memberRight}>
                  ...
                  {isGroupLeader && member.id !== user.id && (
                    <Text style={styles.viewTripsIcon}>📋</Text>
                  )}
                </View>
              </TouchableOpacity>
            );
            ...
            <>
              ...
              >
                <Text style={styles.reportButtonText}>
                  {exporting ? '⏳ Generating...' : '📋 Export Team Trip Report'}
                </Text>
                <Text style={styles.reportButtonSub}>Export all team members' trips to Excel</Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={[styles.transferBtn, { marginTop: 10 }]}
                onPress={openTransferModal}
                disabled={processing}
              >
                <Text style={styles.transferBtnText}>👑 Transfer Leadership</Text>
                <Text style={styles.transferBtnSub}>Pass the leader role to another member</Text>
              </TouchableOpacity>
            </>
          )}
        </View>
      ...
      {isGroupLeader && activeSection === 'subscription' && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>💳 Group Subscription</Text>
          ...
          <TouchableOpacity
            ...
            onPress={() => handlePayment('Group Basic')}
          >
          ...
          <TouchableOpacity
            ...
            onPress={() => handlePayment('Group Pro')}
          >
            <View style={styles.planInfo}>
      ...
      {showMemberTrips && selectedMember && (
        <Modal visible={true} animationType="slide" transparent={false}>
          <View style={styles.modalContainer}>
            <View style={styles.modalHeader}>
              <TouchableOpacity onPress={() => setShowMemberTrips(false)}>
                <Text style={styles.modalBack}>← Back</Text>
              </TouchableOpacity>
              <Text style={styles.modalTitle}>{selectedMember.full_name || selectedMember.email}'s Trips</Text>
              <View style={{ width: 50 }} />
            </View>
            <ScrollView style={styles.modalContent}>
              {memberTrips.length === 0 ? (
                <Text style={styles.emptyText}>No trips found for this member</Text>
              ) : (
                memberTrips.map((trip) => (
                  <View key={trip.id} style={styles.tripCard}>
                    ...
                    <Text style={styles.tripDetails}>
                      📍 {trip.from_address || 'N/A'} → {trip.to_address || 'N/A'}
                    </Text>
                    <Text style={styles.tripDistance}>📏 {trip.distance_km || 0} km</Text>
                    <Text style={styles.tripPurpose}>🎯 {trip.purpose || 'General'}</Text>
                  </View>
                ))
              )}
            </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={{ color: '#666', marginBottom: 12 }}>
              Choose a member to become the new group leader. You will become a regular member.
            </Text>
              ...
              {groupMembers
                ...
                .map((member) => (
                  <TouchableOpacity
                    ...
                    onPress={() => handleTransferLeadership(member)}
                    disabled={processing}
                  >
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.transferMemberEmail}>{member.email}</Text>
                    </View>
                    <Text style={styles.transferMemberArrow}>→</Text>
                  </TouchableOpacity>
                ))}
            </ScrollView>
            <TouchableOpacity
              style={[styles.btn, { backgroundColor: '#6c757d', marginTop: 12 }]}
...
const styles = StyleSheet.create({
  ...
  iconDangerText: { fontSize: 18 },
  groupName: { fontSize: 24, fontWeight: 'bold', color: '#1a1a1a', flex: 1, marginRight: 8 },
  groupDescription: { fontSize: 14, color: '#666', marginTop: 4 },
  groupLeader: { fontSize: 14, color: '#666', marginTop: 8 },
  leaderBadge: {
  ...
  groupMembersCount: { fontSize: 14, color: '#666', marginTop: 4 },
  ...
  actionBtnText: { fontSize: 14, color: '#fff', fontWeight: 'bold' },
  memberCard: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  memberEmail: { fontSize: 12, color: '#999', marginRight: 8 },
  viewTripsIcon: { fontSize: 16, color: '#007AFF' },
  planCard: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  reportButtonText: { fontSize: 16, fontWeight: 'bold', color: '#2e7d32' },
  reportButtonSub: { fontSize: 12, color: '#666', marginTop: 2 },
  ...
  transferBtnText: { fontSize: 16, fontWeight: 'bold', color: '#6f42c1' },
  transferBtnSub: { fontSize: 12, color: '#666', marginTop: 2 },
  ...
  modalBack: { fontSize: 16, color: '#007AFF' },
  ...
  transferMemberName: { fontSize: 15, fontWeight: 'bold', color: '#333' },
  transferMemberEmail: { fontSize: 12, color: '#888' },
  transferMemberArrow: { fontSize: 18, color: '#6f42c1' },
  ...
  tripDate: { fontSize: 14, fontWeight: 'bold', color: '#333' },
  tripDetails: { fontSize: 12, color: '#555', marginTop: 2 },
  tripDistance: { fontSize: 12, color: '#007AFF', marginTop: 2 },
  tripPurpose: { fontSize: 12, color: '#888', marginTop: 2 },
