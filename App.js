...
const TIER_CONFIG = {
  'Group Free': { limit: 30, excel: false, price: '$0/mo', group: true, db_tier: 'team_free' },
...
// Matches the Python mileage-tracker backend
// ============================================================
...
async function getDrivingDistance(lat1, lon1, lat2, lon2) {
  ...
  try {
    ...
    const maxDistance = Math.max(...data.routes.map(r => r.distance)) / 1000;
    console.log('✅ OSRM distance:', maxDistance.toFixed(2), 'km');
    return Math.round(maxDistance * 100) / 100;
  } catch (error) {
    console.warn('OSRM error, using straight-line:', error.message);
    return getStraightLineDistance(lat1, lon1, lat2, lon2);
  }
}
...
export default function App() {
  ...
  const loadTrips = useCallback(async (userId, teamId, role) => {
    try {
      ...
      const formattedTrips = (data || []).map(trip => ({
        ...
        time: trip.time_range || '00:00 - 00:00',
  ...
  const checkSubscriptionStatus = useCallback(async (teamId) => {
    try {
      ...
      const tierMap = {
        'personal_free': 'Group Free',
        'team_free': 'Group Free',
      ...
      return {
        tier: tierMap[data.subscription_tier] || 'Group Free',
  ...
  const loadUserData = useCallback(async (user) => {
    ...
    try {
      ...
      if (isGroupLeaderCheck && currentTeamId) {
        ...
        console.log('📨 Fetched pending count for team:', currentTeamId);
      } else {
        setPendingRequestsCount(0);
      }
  ...
  // ============================================================
  // AUTH EFFECT (FIXED: now properly closed)
  // ============================================================
  useEffect(() => {
    let isMounted = true;
  ...
  // ============================================================
  useEffect(() => {
    console.log("📡 Effect running. user:", user?.id, "teamId:", teamId, "isAdmin:", isAdmin);
    if (!user?.id || !teamId || !isAdmin) {
  ...
  const syncTripToSupabase = async (tripData) => {
    try {
      ...
      const insertData = {
        ...
        time_range: tripData.time,
        start_lat: tripData.startLat ?? null,
        start_lon: tripData.startLon ?? null,
        end_lat: tripData.endLat ?? null,
        end_lon: tripData.endLon ?? null
      };
  ...
  const endTrip = async () => {
    try {
      ...
      const tripData = {
        ...
        userName: driverName,
        startLat: route[0]?.latitude ?? null,
        startLon: route[0]?.longitude ?? null,
        endLat: route[route.length - 1]?.latitude ?? null,
        endLon: route[route.length - 1]?.longitude ?? null,
      };
      ...
      if (savedTrip) {
        ...
        console.log('🎁 App.js: calling checkAndRewardReferral for', user.id);
        const rewardResult = await ReferralService.checkAndRewardReferral(user.id);
        console.log('🎁 App.js: reward result =', JSON.stringify(rewardResult));
      }
...
// ============================================================
// SUM-OF-HOPS DISTANCE CALCULATION (with filters)
// ============================================================
const calculateDistance = (coords) => {
  ...
  for (const cur of coords) {
    if (typeof cur.accuracy === 'number' && cur.accuracy > 30) {
      skippedAccuracy++;
      continue;
    }
    ...
    if (prev) {
      ...
      const φ2 = cur.latitude * Math.PI / 180;
      const Δφ = (cur.latitude - prev.latitude) * Math.PI / 180;
      const Δλ = (cur.longitude - prev.longitude) * Math.PI / 180;
      const a = Math.sin(Δφ/2) * Math.sin(Δφ/2) +
                Math.cos(φ1) * Math.cos(φ2) *
                Math.sin(Δλ/2) * Math.sin(Δλ/2);
      const hop = R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
      ...
      const t1 = prev.timestamp || 0;
      const t2 = cur.timestamp || 0;
      if (t1 && t2) {
        const dtSec = (t2 - t1) / 1000;
        if (dtSec > 0 && hop / dtSec > 55) {
          skippedSpike++;
          continue;
        }
      }
  ...
  console.log(`📏 Sum-of-hops: ${km} km | hops=${hops}, skipped=${skippedAccuracy + skippedJitter + skippedSpike} (acc=${skippedAccuracy}, jitter=${skippedJitter}, spike=${skippedSpike})`);
  return km;
};
      ...
      <View style={{ flex: 1 }}>
        ...
        {activeTab === 'profile' && (
          <ScrollView style={styles.content}>
            ...
            <TouchableOpacity
              style={[styles.btn, { backgroundColor: '#00796b', marginTop: 12 }]}
...
const styles = StyleSheet.create({
  ...
  trackingControls: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 15 },
